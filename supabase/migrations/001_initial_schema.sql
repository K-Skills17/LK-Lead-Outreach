...
CREATE INDEX idx_clinics_license_key ON clinics(license_key);
...
CREATE INDEX idx_campaign_contacts_queue ON campaign_contacts(campaign_id, status) WHERE status = 'pending';
...
CREATE INDEX idx_message_drafts_clinic_id ON message_drafts(clinic_id);
...
CREATE UNIQUE INDEX idx_do_not_contact_phone ON do_not_contact(phone);
...
CREATE INDEX idx_ai_usage_clinic_date ON ai_usage_daily(clinic_id, usage_date);
